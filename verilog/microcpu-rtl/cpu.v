...
parameter ALU_CMD_SIZE=3;  // Αύξηση από 2 σε 3 bits
...
parameter  [OPCODE_SIZE-1:0]  OP_LSL   = 9; //4'b1001 - Logical Shift Left
parameter  [OPCODE_SIZE-1:0]  OP_LSR   = 10; //4'b1010 - Logical Shift Right
...
// Για LSL και LSR, το alu_cmd πρέπει να είναι 3 bits
// Mapping από opcode σε alu_cmd
reg [ALU_CMD_SIZE-1:0] alu_cmd_reg;
always @(opcode)
begin
  case(opcode)
    OP_AND: alu_cmd_reg = 3'b000;
    OP_OR:  alu_cmd_reg = 3'b001;
    OP_XOR: alu_cmd_reg = 3'b010;
    OP_ADD: alu_cmd_reg = 3'b011;
    OP_LSL: alu_cmd_reg = 3'b100;
    OP_LSR: alu_cmd_reg = 3'b101;
    default: alu_cmd_reg = 3'b011; // Default to ADD
  endcase
end
  ...
  if (reset == 1'b1) begin
    ...
    case(state)
      ...
      begin
        ...
        //this is a 3 stages pipelining so IF and DECODE occur on this step
        case(opcode)
        	 OP_AND,OP_OR,OP_XOR,OP_ADD,OP_LSL,OP_LSR:  // Προσθήκη LSL και LSR
          begin
